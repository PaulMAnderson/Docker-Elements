...
if id -u "$USER_NAME" &>/dev/null; then
    ...
    if [ "$CURRENT_UID" -eq "$USER_UID" ] && [ "$CURRENT_GID" -eq "$USER_GID" ]; then
        ...
        # Get the user's home directory and primary group name
...
# Move or symlink the default home directory to the desired user's home
...
if [[ "${USER_NAME}" != "${DEFAULT_USER}" ]]; then
    if [[ ! -e "/home/${USER_NAME}" ]]; then
        echo "Attempting to copy /home/${DEFAULT_USER} to /home/${USER_NAME}..."
        mkdir "/home/${USER_NAME}"
        
        # Fix: Copy all files including hidden ones
        # shellcheck disable=SC2086
        shopt -s dotglob  # Enable including hidden files in glob patterns
        if cp ${CP_OPTS:--a} /home/${DEFAULT_USER}/* "/home/${USER_NAME}/"; then
            # Also copy the hidden files that are directly in the home dir
            cp ${CP_OPTS:--a} /home/${DEFAULT_USER}/.[!.]* "/home/${USER_NAME}/" 2>/dev/null || true
            echo "Success!"
            # Remove the directory
            # echo "Removing the old directory /home/${DEFAULT_USER}..."
            ...
            if ln -s /home/${DEFAULT_USER} "/home/${USER_NAME}"; then
                ...
                echo "ERROR: Failed copy data from /home/${DEFAULT_USER} to /home/${USER_NAME} or to create symlink!"
                exit 1
            fi
        fi
    fi
...
# Change the home environmental variable

HOME="${USER_HOME}"
cd "${HOME}"


## Detect if this is running on Athena and if so run as head Ray node
## Otherwise run as a worker
# HEAD_TAILSCALE_IP=100.80.2.0 # Trying with the tailscale IP
HEAD_PORT=6379
DASHBOARD_PORT=8265

## Detect current machine's IP
CURRENT_IP="${HOST_IP:-$(hostname -I | awk '{print $1}')}"
# echo "Detected IP: $CURRENT_IP"

##  Create appropriate supervisord config based on IP
# if [[ "$CURRENT_IP" == "$HEAD_NODE_IP" ]]; then

# New version where can just specify if head
# Default Ray port
RAY_PORT=${RAY_PORT:-10000}
# Default Ray dashboard port
DASHBOARD_PORT=${DASHBOARD_PORT:-8265}
# Default IP address binding (0.0.0.0 allows external connections)
BIND_ADDRESS=${BIND_IP:-0.0.0.0}
# Default head node address for workers to connect to
HEAD_ADDRESS=${HEAD_IP:-100.80.2.0}

# Check if IS_HEAD is set and equals "true" (case insensitive)
if [[ "${IS_HEAD,,}" == "true" ]]; then
    echo "Starting Ray as head node..."
    cat >> /etc/supervisor/conf.d/supervisord.conf << EOF

[program:ray]
command=ray start --head --dashboard-host=$BIND_ADDRESS --port=$RAY_PORT --dashboard-port=$DASHBOARD_PORT --num-gpus=1 --block --disable-usage-stats --redis-shard-ports=10001 --node-manager-port=10002 --object-manager-port=10003 --gcs-server-port=10004 --min-worker-port=10005 --max-worker-port=10099
autostart=true
autorestart=unexpected
exitcodes=0
startsecs=10
startretries=3
stderr_logfile=/tmp/ray_head.err.log
stdout_logfile=/tmp/ray_head.out.log

EOF
else
    echo "Starting Ray as worker node..."
    cat >> /etc/supervisor/conf.d/supervisord.conf << EOF


[program:ray]
command=ray start --address=$HEAD_ADDRESS:$RAY_PORT  --node-ip-address=$CURRENT_IP --num-gpus=1 --block --disable-usage-stats --node-manager-port=10002 --object-manager-port=10003 --min-worker-port=10005 --max-worker-port=10099
autostart=true
autorestart=unexpected
exitcodes=0
startsecs=10
startretries=3
stderr_logfile=/tmp/ray_worker.err.log
stdout_logfile=/tmp/ray_worker.out.log
